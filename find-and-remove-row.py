...
the requested string in any column to a new *_removed.csv file.

The input path and search string are required. The output file is created beside
the input file automatically.

Usage:
    python find-and-remove-row.py folder/data.csv "example.com"
...
def find_and_remove(input_path: Path, search_string: str) -> tuple[Path, int, int]:
    """Write the header and rows without search_string anywhere in the row."""
    if not input_path.is_file():
        raise FileNotFoundError(f"Input file not found: {input_path}")

    output_path = input_path.with_name(f"{input_path.stem}_removed{input_path.suffix}")
